...
public final class JobEventRdbSearch {
    ...
    public Result<JobExecutionEvent> findJobExecutionEvents(final Condition condition) {
        ...
        final Result<JobExecutionEvent> jobExecutionEventResult = new Result<>(getEventCount(TABLE_JOB_EXECUTION_LOG, FIELDS_JOB_EXECUTION_LOG, condition), getJobExecutionEvents(condition));


        final List<JobExecutionEvent> rows = jobExecutionEventResult.getRows();

        List<JobExecutionEvent> result = new LinkedList<>();


        for (JobExecutionEvent event : rows) {
            if (!Strings.isNullOrEmpty(event.getTaskId())) {
                String[] taskSplits = event.getTaskId().split("@-@");
                if (taskSplits.length > 4) {
                    String ip = taskSplits[3] + "@" + taskSplits[4];
                    event.setIp(ip);
                }
            }
        ...
        return new Result<>(jobExecutionEventResult.total, result);
    }
    ...
    private String buildDataSql(final String tableName, final Collection<String> tableFields, final Condition condition) {
        ...
        log.info("data sql {}" , sqlBuilder.toString());
    ...
    private String buildCountSql(final String tableName, final Collection<String> tableFields, final Condition condition) {
        ...
        log.info("count sql {}" , sqlBuilder.toString());
        return sqlBuilder.toString();
    }
